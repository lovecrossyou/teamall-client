<template>
	<view class="content-wrapper">
		...
		<view class="defult-addr" @click="addrM.defaultAddress = !addrM.defaultAddress">
...
export default {
	computed: {
		...
		defultIcon() {
			return this.addrM.defaultAddress ? '../../static/addr/defult_addr_selected.png' : '../../static/addr/defult_addr.png';
		}
	},
	data() {
		return {
	...
	methods: {
		...
		saveOrUpdate() {
			if (this.addrM.receiveName == '') {
				uni.showToast({
					title: '请填写姓名'
				});
				return;
			}
			if (this.addrM.areaAddress == '') {
				uni.showToast({
					title: '请选择区域'
				});
				return;
			}
			if (this.addrM.phoneNum == '') {
				uni.showToast({
					title: '请填写联系方式'
				});
				return;
			}
			if (this.addrM.detailedAddress == '') {
				uni.showToast({
					title: '请填写详细地址'
				});
				return;
			...
			let params = {
				params: {
					areaAddress: this.addrM.areaAddress,
					detailedAddress: this.addrM.detailedAddress,
					phoneNum: this.addrM.phoneNum,
					receiveName: this.addrM.receiveName,
					defaultAddress: this.addrM.defaultAddress
				},
				callback: () => {
			...
			if(this.addrM.deliveryAddressId == undefined || this.addrM.deliveryAddressId == null){
				this.createAddress(params);
			}else{
				params.params.deliveryAddressId = this.addrM.deliveryAddressId;
		...
		onConfirm(e) {
			...
			this.addrM.areaAddress = e.label.replace(/-/g, '');
		},
		onCancel(e) {
			console.log(e);
		}
